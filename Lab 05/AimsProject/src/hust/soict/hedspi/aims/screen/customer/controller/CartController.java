...
public class CartController {
    ...
    void btnPlaceOrderPressed(ActionEvent event) {
        if (cart.getItemsOrdered().isEmpty()) {
            ...
            alert.setHeaderText(null);
            alert.setContentText("Chưa có sản phẩm nào trong giỏ hàng. HÃy thêm sản phẩm để đặt hàng!");
            alert.showAndWait();
        } else {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Đặt hàng thành công");
            alert.setHeaderText(null);
            alert.setContentText("Chúng tôi đã nhận được đơn hàng của bạn. Bạn đã đặt hàng thành công!");
            alert.showAndWait();
