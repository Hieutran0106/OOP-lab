...
public class CompactDisc extends Disc implements Playable {
	...
	public CompactDisc() {
	 ...
	 public CompactDisc(String title, float cost, String artist) {
	        super(title, cost); // Giả sử category là "Unknown" nếu chưa hỏi
	        this.artist = artist;
	        this.length = 0; // mặc định nếu chưa có length
	    }
	public CompactDisc(String title, String category, float cost, String director, int length, String artist) {
        super(++nbCompactDiscs, title, category, director, length, cost); 
        this.artist = artist;
	...
	public String toString() {
	    return "CompactDisc [id=" + getId() + ", title=" + getTitle()    + ", category=" + getCategory() + ", cost=" + getCost()  + ", length=" + getLength()  + ", director=" + getDirector() + ", artist=" + artist  + ", number of tracks=" + tracks.size()+ "]";
	}
	@Override
